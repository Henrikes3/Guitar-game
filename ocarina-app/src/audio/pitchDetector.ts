...
// A 12-hole ocarina typically spans roughly A4 (~440Hz) to somewhere around
// D6-F6 with normal and overblown fingerings; sopranino/soprano models sit a
// bit higher. We widen the window a bit below/above that for flat singing,
// slightly-off instruments, and detection noise.
const MIN_FREQUENCY = 380;
 ...
 * tracking, which fits an ocarina (or a hummed/sung melody) perfectly since it
 * only ever sounds one note at a time.
 */
...
export interface PitchFrame {
   ...
   *  regardless of whether a clean pitch was found, so callers (like the
   *  melody transcriber) can use it for note-onset/silence detection even
   *  during the noisy attack of a breath or hum. */
  level: number;
